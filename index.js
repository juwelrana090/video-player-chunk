/**
 * Local Video Player – Backend
 * Direct local video playback with no upload functionality.
...
app.get('/local-stream', (req, res) => {
  ...
  const cmd = ffmpeg(fp)
    ...
    .addOption('-map', '0:V') // map all video streams (skip attached pictures)
    .videoCodec('libx264')
    ...
    .addOption('-level', '3.0')
    .outputOptions([
      '-movflags', 'frag_keyframe+empty_moov+dash+cmaf', // better browser compatibility
  ...
  if (audioIdx >= 0) {
    cmd
      ...
      .audioCodec('aac')
      .addOption('-b:a', '128k') // lower bitrate for better compatibility
      .addOption('-ac', '2')
      .addOption('-ar', '44100') // use 44.1kHz for better browser support
      .addOption('-strict', 'experimental'); // ensure AAC compatibility
  } else {
    cmd.addOption('-an');
  }
  ...
  cmd
    ...
    .on('error', (err) => {
      ...
      console.error(`[local #${sid}] Error:`, err.message);
      if (err.stderr) console.error(`[local #${sid}] stderr:`, err.stderr.slice(-800));
...
app.listen(PORT, () => {
  ...
  console.log(`📁  Browse local PC folders directly\n`);
});